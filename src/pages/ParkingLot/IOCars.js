		...
		return (
            <div id="IO-Cars">
                <div className="input">
                    ...
                    <div>{this.props.parkingLotData._IOCars.inputCars.map((item, i) => <p key={item.id+item.time} className={`list ${item.New ? 'new' : ''}`}><span className="col-lg-4">{item.license}</span><span className="col-lg-4">{item.addr}</span><span className="col-lg-4">{item.time}</span></p>)}</div>
                </div>
                <div className="output">
                    ...
                    <div>{this.props.parkingLotData._IOCars.outputCars.map((item, i) => <p key={item.id+item.time} className={`list ${item.New ? 'new' : ''}`} ><span className="col-lg-4">{item.license}</span><span className="col-lg-4">{item.time}</span><span className="col-lg-4">{item.stayTime} 分钟</span></p>)}</div>
                </div>
		    </div>
        );
	}
